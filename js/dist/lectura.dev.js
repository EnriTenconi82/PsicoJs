...
function showMessages(arrayIn, esLeido) {
  ...
  alert('Se mostraran  mensajes ' + texto + ' para leer.'); //filtro a array los mensajes que cumplen condicion
  ...
  if (mensMonstrados.length > 0) {
    noMessage = false;
    mensMonstrados.forEach(function (elemento) {
...
function muestraMensNuevos() {
  ...
  mensajes.forEach(function (elemento, i) {
    if (elemento.leido == false) {
      do {
        //quiero marcar el mensaje como leido?
      ...
      } while (validator == false); //si marco si marco como leido


      if (temp == "S" || temp == "s") mensajes[i].leido = true; //subo a serv de modificaciones
